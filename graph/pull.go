...
func (s *TagStore) CmdPull(job *engine.Job) engine.Status {
	...
	if is_official || endpoint.Version == registry.APIVersion2 {
		...
		err = s.pullV2Repository(job.Eng, r, job.Stdout, localName, remoteName, tag, sf, job.GetenvBool("parallel"))
		if err == nil {
			return engine.StatusOK
...
func (s *TagStore) pullV2Repository(eng *engine.Engine, r *registry.Session, out io.Writer, localName, remoteName, tag string, sf *utils.StreamFormatter, parallel bool) error {
	out.Write(sf.FormatStatus("", "Pulling V2 repository %s", localName))

	if tag == "" {
		log.Debugf("Pulling tag list for %s", remoteName)
		tags, err := r.GetV2RemoteTags(remoteName, nil)
		if err != nil {
			return err
		}
		for _, t := range tags {
			err = s.pullV2Tag(eng, r, out, localName, remoteName, t, sf, parallel)
			if err != nil {
				return err
			}
		}
	} else {
		err := s.pullV2Tag(eng, r, out, localName, remoteName, tag, sf, parallel)
		if err != nil {
			return err
		}
	}
...
func (s *TagStore) pullV2Tag(eng *engine.Engine, r *registry.Session, out io.Writer, localName, remoteName, tag string, sf *utils.StreamFormatter, parallel bool) error {
	...
	for i := len(downloads) - 1; i >= 0; i-- {
		...
		if d.downloaded {
			...
			if d.tmpFile != nil {
				...
				// FIXME: Pool release here (ensures any downloads block until fully extracted?
			}
	...
	//	if err = s.Set(localName, tag, d.img.ID, true); err != nil {
	//		return err
	//	}
	//}
	return nil
