...
func (s *TagStore) CmdPull(job *engine.Job) engine.Status {
	...
	if len(mirrors) == 0 && (isOfficial || endpoint.Version == registry.APIVersion2) {
		...
		auth, err := r.GetV2Authorization(remoteName, true)
		if err != nil {
...
func (s *TagStore) pullV2Repository(eng *engine.Engine, r *registry.Session, out io.Writer, localName, remoteName, tag string, sf *utils.StreamFormatter, parallel bool, auth *registry.RequestAuthorization) error {
	var layersDownloaded bool
	if tag == "" {
		log.Debugf("Pulling tag list from V2 registry for %s", remoteName)
		tags, err := r.GetV2RemoteTags(remoteName, auth)
		if err != nil {
			return err
		}
		for _, t := range tags {
			if downloaded, err := s.pullV2Tag(eng, r, out, localName, remoteName, t, sf, parallel, auth); err != nil {
				return err
			} else if downloaded {
		...
		if downloaded, err := s.pullV2Tag(eng, r, out, localName, remoteName, tag, sf, parallel, auth); err != nil {
			return err
		} else if downloaded {
			layersDownloaded = true
		}
	...
	for i := len(manifest.FSLayers) - 1; i >= 0; i-- {
		...
		downloadFunc := func(di *downloadInfo) error {
			...
			if c, err := s.poolAdd("pull", "img:"+img.ID); err != nil {
				...
				r, l, err := r.GetV2ImageBlobReader(remoteName, sumType, checksum, auth)
				if err != nil {
					return err
				}
				defer r.Close()
